...
export default function Header() {
  ...
  return (
    <header className="z-[99] relative">
      <motion.div
       className="fixed top-0 left-1/2 h-[4.5rem] w-full rounded-none border border-[#fdfdfc] border-opacity-40 bg-black shadow-lg shadow-black/[0.03] backdrop-blur-[0.5rem] sm:top-6 sm:h-[3.25rem] md:w-[41rem] md:rounded-full"
      ...
      <nav className="flex fixed top-[0.15rem] left-1/2 h-12 -translate-x-1/2 py-2 sm:top-[1.7rem] sm:h-[initial] sm:py-0">
        <ul className="flex w-[22rem] flex-wrap items-center justify-center gap-y-2 text-[0.9rem] font-medium sm:w-[initial] sm:flex-nowrap sm:gap-5 transition-colors">
          {links.map((link) => (
            <motion.li
            className="h-3/4 flex items-center justify-center relative text-white dark:text-white"
              key={link.id}
            ...
            >
              ...
              >
                ...
                {link.id === activeSection && (
                  <motion.span
                    className="rounded-full absolute inset-0 -z-10 dark:bg-[#f5f4f4] bg-[#6296e4]"
